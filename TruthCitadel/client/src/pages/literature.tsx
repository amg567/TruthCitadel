...
export default function Literature() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/content/literature"] });
      ...
      toast({
        title: "Success",
        description: "Literature entry created successfully",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to create literature entry",
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Literature entry updated successfully",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to update literature entry",
        variant: "destructive",
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Literature entry deleted successfully",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to delete literature entry",
        variant: "destructive",
      });
    },
  });
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-academia-brown/50 border-b border-academia-gold/20 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-serif font-bold text-academia-gold">Literature</h2>
              <p className="text-academia-beige/70 mt-1">Your collection of literary works and insights</p>
            </div>
              ...
              <DialogContent className="bg-academia-dark border-academia-gold/20">
                <DialogHeader>
                  <DialogTitle className="text-academia-gold">
                    {editingEntry ? "Edit Literature Entry" : "Add Literature Entry"}
                  </DialogTitle>
                </DialogHeader>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-academia-beige">Content</FormLabel>
                          <FormControl>
                            <Textarea 
                              placeholder="Your thoughts, quotes, analysis, or notes..."
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input 
                              placeholder="fiction, philosophy, classic"
                              {...field}
                              className="bg-academia-brown/30 border-academia-gold/20 text-academia-cream"
                            />
                          </FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-academia-beige">Image URL (optional)</FormLabel>
                          <FormControl>
                            <Input 
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              <Book className="w-16 h-16 text-academia-gold/50 mx-auto mb-4" />
              <h3 className="text-xl font-serif text-academia-gold mb-2">No literature entries yet</h3>
              <p className="text-academia-beige/70 mb-6">Start building your literary collection by adding your first entry.</p>
              <Button 
